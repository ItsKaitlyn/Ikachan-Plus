...
BOOL Flip_SystemTask(HWND hWnd)
{
	...
	while (GetTickCount() < (timePrev + framespeed))
	{
		Sleep(1);
		if (!SystemTask())
			return FALSE;
...
//Surface creation
BOOL MakeSurface_File(const char* name, int surf_no) //TODO: ASM accuracy
{
	//Get path
	char path[MAX_PATH];
...
void PutText2(int x, int y, const char* text, DWORD color, int surf_no, BOOL unk)
{
	//Lock surface
	HDC hdc;
